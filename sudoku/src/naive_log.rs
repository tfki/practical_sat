...
use solver::{dimacs_emitting, ipasir, LitValue, SatProblemResult, Solver, SolverImpl, SolveWithTimeoutResult};
    ...
    let grid = grid.into_iter()
        .map(|col| col.into_iter()
            .map(|vals| vals.into_iter()
                .map(|val| if val { Some(solver.new_lit()) } else { None })
                .collect())
            .collect())
        .collect();
...
fn encode<T: SolverImpl>(sudoku: &Sudoku, mut solver: Solver<T>) -> (Solver<T>, HashMap<String, Lit>) {
    let mut var_map = HashMap::<String, Lit>::new();
    let mut sudoku = sudoku.clone();

    let bits = (sudoku.n.pow(2) as f32).log2().ceil() as u32;

    let (possible_value_grid, _new_comments) = build_value_grid_and_optimize(&mut sudoku, &mut solver, &mut var_map);
...
pub fn find_solution(sudoku: &Sudoku, timeout: Duration) -> SatProblemResult<Sudoku> {
    let solver = Solver::<ipasir::Solver>::new();
...
pub fn gen_dimacs(sudoku: &Sudoku) -> String {
    ...
    let solver = Solver::<dimacs_emitting::Solver>::new();
    let (mut solver, var_map) = encode(&sudoku, solver);
