...
use solver::{dimacs_emitting, ExactlyKStrategy, ipasir, LitValue, SatProblemResult, Solver, SolveResult};
...
pub fn gen_dimacs(input: Input) -> String {
    ...
    for t in CellContent::all_values() {
        for x in 0..input.width.saturating_sub(t.dimensions().0.saturating_sub(1)) {
            for y in 0..input.height.saturating_sub(t.dimensions().1.saturating_sub(1)) {
                var_map.insert(format!("{x}/{y} is {t:?} anchor"), solver.new_lit());
            }
        }
    }
    ...
    let mut entries = var_map.iter().collect::<Vec<(&String, &Lit)>>();
    entries.sort_by_key(|(_, lit)| lit.var.id);

    for (key, lit) in entries {
        solver.implementation.add_comment(format!("{} iff {key}", lit.var.id));
    }
